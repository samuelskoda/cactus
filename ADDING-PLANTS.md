...
| `file` | Path to the photo inside the plant's own folder: `photos/<id>/IMG_XXXX.webp`. Safe to swap later (e.g. after a reshoot) — the `id` stays fixed. Photos are **WebP**, in two sizes. Export from Photos as JPEG (sRGB, width 1600), then make both:
`cwebp -q 82 -resize 1200 0 in.jpeg -o photos/<id>/IMG_XXXX.webp` (detail view) and `cwebp -q 80 -crop <x> <y> <side> <side> -resize 600 600 in.jpeg -o photos/<id>/IMG_XXXX-t.webp` (card — a centred SQUARE crop, since the card is square; `side` = the shorter edge). Only the plant's `file` photo needs a `-t` thumbnail. Always convert from the export, never from an already-web-sized file, so you are not stacking two lossy passes. The `-t` path is derived in code, so it is not listed in data.js. |
